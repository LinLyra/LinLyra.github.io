...
import { Rocket, ArrowLeft, Award, Trophy, BriefcaseBusiness } from "lucide-react";
...
export default function ProductPage() {
  ...
  const products: ProductItem[] = [
    ...
    // 赛事：奖杯 + 全国亚军
    {
      ...
      description: "Global university challenge by Deloitte China on digital product innovation.",
    ...
    // 赛事：奖杯
    {
      ...
      description: "2nd Global AI Hackathon (co-hosted with MIT Sloan AI Club).",
    ...
    // 赛事：奖杯 + Top 9
    {
      ...
      placement: "Top 9",
      description: "Building social-impact solutions — accessible films & product design.",
    ...
    // 赛事：奖杯
    {
      ...
      description: "China’s largest youth-driven hackathon — rapid product prototyping.",
  ...
  const filtered = products.filter((p) => {
    const bag = [p.title, p.company ?? "", p.event ?? "", p.description ?? "", ...(p.tags ?? []), ...(p.skills ?? [])]
      .join(" ")
      .toLowerCase();
  ...
  const MetaIcon = ({ type }: { type: ProductType }) => {
    if (type === "development") return <BriefcaseBusiness className="h-4 w-4" />;
    if (type === "project") return <Rocket className="h-4 w-4" />;
      ...
      <div className="relative z-10 pt-20 p-6">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filtered.map((p) => {
              const hasVisual = Boolean(p.image || p.logo);
              return (
                  ...
                  <Card className="min-h-[220px] cursor-pointer overflow-hidden border-amber-400/20 bg-black/30 backdrop-blur-md transition-all duration-300 hover:bg-black/40">
                    <CardHeader className="pb-2">
                      <div className={`flex items-start justify-between`}>
                        <div className={`flex items-center ${hasVisual ? "gap-3" : "gap-0"}`}>
                          {/* 仅在有图时渲染 Logo 方块；否则不占位 */}
                          {hasVisual && (
